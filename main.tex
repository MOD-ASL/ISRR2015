...
\subsubsection{Verification of Configurations \TODO{Consider removing these
subsection headings to save space}}\label{sec:verify_conf}
...
To verify a behavior with time duration  $T_B$, we discretize execution with sampling time $t_B$. At each time step, we first detect behavior conflict by checking if different commands are given to the same joint of a module simultaneously. If there is no behavior conflict, we update the positions and orientations of all modules in the configuration based on the commands. We then check for self-collision and gravitational instability, using the methods described in section~\ref{sec:verify_conf}.  
...
and associated behaviors in Figure~\ref{fig:library}.
