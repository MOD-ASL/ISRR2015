...
programming them \cite{salemi2001hormone, walter2002choosing,
melek2003neurofuzzy}.  Over 800 papers, a book \cite{stoy2010book}, and a survey \cite{yim2007modular} have been written on the subject.
...
{\color{red}In this paper, we consider only configurations without cycles. While this is a constraint,
...
with cycles is left to future work.} \TODO{We are removing this requirement, right?}
...
In this section, we present a programming paradigm for creating new behaviors for
modular robots by composing existing ones.  We wanted to allow
users to create  designs and behaviors quickly and easily,  enabling them
address tasks by generating a new robot and behavior on-the-fly.
...
to specifying the kind of non-cyclic, globally coordinated behaviors required for many tasks
...
As an example, consider the car design shown in figure \ref{fig:car-design}. To create a low-level
...
18-module backhoe shown in Figure \ref{fig:backhoe-design} by composing  behaviors of
...
\sidecaption
\begin{center}
\includegraphics[width=0.4\columnwidth]{images/walkbot_collision.png}
\end{center}
\caption{A configuration with self-collision \TODO{If space becomes an issue, we
might consider taking this figure out.}}
...
be in contact with the ground plane, and treat the
set \( \sigma = P(\mathcal{M}), \forall \mathcal{M} \in M_c \ \) as an approximate set of ground
contact points.
...
\begin{itemize}
\item Module do not collide at any time during the execution of the behavior.
...
\begin{figure}[tb]
    \sidecaption
    \begin{center}
        \includegraphics[width=0.5\columnwidth]{images/smores_robot.png}
    \end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.5\columnwidth]{images/tikz/design_diagram.pdf}
